...
public class CreateScatterPlots {
	...
	public static void run(final String file1, final String matrixID1, final String file2, final String matrixID2,
			final String scatterFile) throws FileNotFoundException {
		...
		final CoordinateTransformation zone2netTrafo = StockholmTransformationFactory.getCoordinateTransformation(
				StockholmTransformationFactory.WGS84_EPSG3857, StockholmTransformationFactory.WGS84_SWEREF99);
		...
		for (Zone zone : zonalSystem.getId2zoneView().values()) {
			...
			{
				final Coordinate zoneCentroidCoordinate = zone.getGeometry().getCentroid().getCoordinate();
				centroid = MGC.coord2Point(
						zone2netTrafo.transform(new Coord(zoneCentroidCoordinate.x, zoneCentroidCoordinate.y)));
			}

			if (plotAll || (centroid.getCoordinate().x >= xMin && centroid.getCoordinate().x <= xMax
					&& centroid.getCoordinate().y >= yMin && centroid.getCoordinate().y <= yMax)) {
		...
		for (List<Entry> row1 : xMatrix.getFromLocations().values()) {
			for (Entry entry1 : row1) {
				...
				if (centralZoneIDs.contains(entry1.getFromLocation())
					...
					final Entry entry2 = yMatrix.getEntry(entry1.getFromLocation(), entry1.getToLocation());
					if (entry2 != null) {
	...
	public static void main(String[] args) throws FileNotFoundException {
		...
		run("./test/referencedata/EMME_traveltimes_OTHER_mf9.xml", null, "./test/matsim-testrun/tourtts.xml", "OTHER",
