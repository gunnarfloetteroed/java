...
/**
 * This is a copy of the original MATSim class with a PUBLIC constructor, which appears necessary to
 * evaluate the tolls of hypothetical (i.e. not mobsim-simulated) travel plans.
 * 
 * Calculates the toll agents pay during a simulation by analyzing events. Add
 * an instance of this class as an EventHandler.
 * 
 ...
 * @author mrieser
 */
public final class RoadPricingTollCalculator
	...
	/**
	 * Much of this is no longer needed since we now throw money events immediately
	 * when links are left. It is, however, still needed for area toll, and for the
	 * specific implementation of cordon toll here. kai, jan'21
	 */
	...
	// TODO Sole difference to contrib class: making this constructor public
	// RoadPricingTollCalculator(final Network network, final RoadPricingScheme
	// scheme, EventsManager events ) {
	public RoadPricingTollCalculator(final Network network, final RoadPricingScheme scheme, EventsManager events) {
		super();
		...
		if (RoadPricingScheme.TOLL_TYPE_DISTANCE.equals(scheme.getType())) {
			...
			throw new RuntimeException(
					"use LINK toll behavior to implement cording pricing (just pay on links that traverse the cordon)");
	...
	public void handleEvent(final VehicleEntersTrafficEvent event) {
		...
		if (handler instanceof DistanceTollBehaviour || handler instanceof LinkTollBehaviour) {
			/*
			 * we do not handle wait2link-events for these tolls, because the agent should
			 * not pay twice for this link, and he (likely) paid already when arriving at
			 * this link.
			 */
			...
			this.handler.handleEvent(new LinkEnterEvent(event.getTime(), event.getVehicleId(), event.getLinkId()),
					link);
		}
	}
...
//	}
	...
	/**
	 * Handles the calculation of the distance toll. If an agent enters a link at a
	 * time the toll has to be paid, the toll amount is added to the agent. The
	 * agent does not have to pay the toll for a link if it starts on the link, as
	 * it may have paid already when arriving on the link.
	 */
	private class DistanceTollBehaviour implements TollBehaviourI {
		...
		public void handleEvent(final LinkEnterEvent event, final Link link) {
			...
			Cost cost = RoadPricingTollCalculator.this.scheme.getLinkCostInfo(link.getId(), event.getTime(), driverId,
					event.getVehicleId());
			if (cost != null) {
				...
				AgentTollInfo info = RoadPricingTollCalculator.this.agents.computeIfAbsent(driverId,
						(key) -> new AgentTollInfo());
				info.toll += newToll;
			}
		}
	}
	...
	/**
	 * Handles the calculation of the area toll. Whenever the agent is seen on one
	 * of the tolled link, the constant toll amount has to be paid once. <br>
	 * Design comments:
	 * <ul>
	 * <li>This implementation becomes a problem if someone tries to implement more
	 * than one area which do not share the same flat fee. kai, mar'12
	 * </ul>
	 */
		...
		public void handleEvent(final LinkEnterEvent event, final Link link) {
			...
			if (cost != null) {
				...
				if (info == null) {
					...
					/*
					 * The toll amount comes from the current link, but should be the same for all
					 * links.
					 */
					events.processEvent(new PersonMoneyEvent(event.getTime(), driverId, -cost.amount, "toll", null));
