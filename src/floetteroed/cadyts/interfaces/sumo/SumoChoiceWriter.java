...
import static floetteroed.cadyts.interfaces.sumo.SumoPopulationHandler.CHOICEPROB_ATTR;
import static floetteroed.cadyts.interfaces.sumo.SumoPopulationHandler.DEPART_ATTR;
import static floetteroed.cadyts.interfaces.sumo.SumoPopulationHandler.EDGES_ATTR;
import static floetteroed.cadyts.interfaces.sumo.SumoPopulationHandler.EXITTIMES_ATTR;
import static floetteroed.cadyts.interfaces.sumo.SumoPopulationHandler.FROMTAZ_ATTR;
import static floetteroed.cadyts.interfaces.sumo.SumoPopulationHandler.ROUTE_ELEM;
import static floetteroed.cadyts.interfaces.sumo.SumoPopulationHandler.TOTAZ_ATTR;
import static floetteroed.cadyts.interfaces.sumo.SumoPopulationHandler.VEHICLEID_ATTR;
import static floetteroed.cadyts.interfaces.sumo.SumoPopulationHandler.VEHICLE_ELEM;
	...
	public void write(final SumoAgent agent, final SumoPlan plan)
		...
		if (agent.getMiscAttrs() != null) {
			for (Tuple<String, String> miscAttr : agent.getMiscAttrs()) {
				writeAttr(miscAttr.getA(), miscAttr.getB(), this.writer);
			}
		}
		...
		// >>>>> TODO BUGFIX STARTING HERE >>>>>
		String lastLink = plan.getStartLink();
		for (PlanStep<String> step : plan.getPlan4fileDump()) {
		...
		// <<<<< TODO BUGFIX ENDING HERE <<<<<

		// >>>>> TODO ORIGINAL STARTING HERE >>>>>
		// String lastLink = plan.getStartLink();
