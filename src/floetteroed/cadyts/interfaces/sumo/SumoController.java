...
import static floetteroed.cadyts.interfaces.sumo.SumoCalibrator.DEFAULT_CLONE_POSTFIX;
import static floetteroed.cadyts.interfaces.sumo.SumoCalibrator.DEFAULT_OVERRIDE_TRAVELTIMES;
...
public class SumoController extends
	...
	protected PopulationFileReader<SumoAgent> newPopulationReader(
		...
		SumoPopulationHandler populationHandler = new SumoPopulationHandler(
				calibrator.newPlanBuilder(), calibrator.getTravelTimes(),
				calibrator.getDemandScale());
		reader.setPopulationHandler(populationHandler);

		if (calibrator.getDemandScale() > 1.0) {
			final SumoAgentCloner cloner = new SumoAgentCloner(calibrator
					.newPlanBuilder());
