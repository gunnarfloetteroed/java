...
/**
 * This is based on
 * {@link org.matsim.core.scoring.functions.SubpopulationScoringParameters} .
 * The difference is that the "params" field is recreated in each iteration,
 * thus being able to pick up changes inserted into the config scoring
 * parameters by Opdyts.
 * 
...
public class EveryIterationScoringParameters implements ScoringParametersForPerson {
	...
	EveryIterationScoringParameters(PlansConfigGroup plansConfigGroup,
		...
		// 2020-08-14: removed while moving to MATSim 12
	...
	public ScoringParameters getScoringParameters(Person person) {
		...
		final String subpopAttribName = (String) person.getAttributes().getAttribute(subpopulationAttributeName);

		if (!this.params.containsKey(subpopAttribName)) {
			/*
			 * lazy initialization of params. not strictly thread safe, as different threads
			 * could end up with different params-object, although all objects will have the
			 * same values in them due to using the same config. Still much better from a
			 * memory performance point of view than giving each ScoringFunction its own
			 * copy of the params.
			 */
		...
		if (flag) {
			flag = false;
			for (Entry<String, ModeUtilityParameters> entry : this.params.get(subpopAttribName).modeParams.entrySet()) {
